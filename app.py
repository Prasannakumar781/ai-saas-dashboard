...
def sanitize_records(records):
    """
    Walk every value in every row and convert float NaN / Inf / -Inf to None.
    pandas .where() and .replace() are unreliable for this — row-by-row is bulletproof.
    """
    clean = []
    for row in records:
        clean_row = {}
...
if file:
    try:
        ...
        records = sanitize_records(records)  # removes ALL NaN/Inf before JSON encoding
